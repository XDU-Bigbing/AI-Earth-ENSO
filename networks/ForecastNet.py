...
class ForecastNetPlus(nn.Module):
    ...
    def __init__(self, encoder, regressor, decoder, sliding_window_size,
                 output_seq_length, device):
        """
        Constructor
    ...
    def forward(self, input_seq, is_training=True):
        ...
        """
        # Initialise outputs
        outputs_x = torch.zeros(
            (input_seq.shape[0], input_seq.shape[1], self.output_seq_length,
             input_seq.shape[3], input_seq.shape[4])).to(self.device)
        outputs_y = torch.zeros(
            (input_seq.shape[0], self.output_seq_length)).to(self.device)
        ...
        for i in range(self.output_seq_length):
            ...
            if is_training:
                next_cell_input = input_seq[:, :, i + 1:i +
                                            self.sliding_window_size + 1, :, :]
            else:
                if i + 1 < input_seq_length:
                    next_cell_input = torch.cat(
                        (input_seq[:, :, i + 1:input_seq_length, :, :],
                         outputs_x[:, :, :i + 1, :, :]),
                        dim=2)
                else:
                    next_cell_input = outputs_x[:, :, i + 1 -
                                                self.sliding_window_size:i +
                                                1:, :]
